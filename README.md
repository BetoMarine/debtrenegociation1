...
Three **separate** free Hong Kong consumer tools from Plan Your Life (PoC v0.9.12). Each has its own URL, Safari Add-to-Home-Screen title, and manifest. There is **no shared chooser**. Shared brand: purple `#7e22ce`, teal `#06b6d4`, purple→teal primary CTA, light canvas, small **Plan Your Life** wordmark. Legal disclaimers stay in compact footers / privacy cards.
...
| **Plan Your Life DRAFT shelf** (QA only — production planyourlife.tech not swapped; two live tools, Fortune testing) | https://betomarine.github.io/debtrenegociation1/pyl/ | — |
...
7. Fortune Teller: every open lands on Step 1 (Me today) after Privacy is accepted once. Returning visitors confirm with **Looks right — open my plan** (no auto-skip to the board). Rebuild card first; Step 2 is card-specific (fire links + floor for rebuild). Fire links to Right Door / Sunday Pack carry `?from=fortune`. Those apps show a persistent **Back to Fortune Teller** bar (not on standalone opens). On return, Fortune **receives** a Fix milestone created by the pack (debt renegotiation 3 or 6 months — pick once if the pack had no tenor). Emergency fund is a mandatory Stabilize milestone (current amount may be 0). **All** Plan/Invest goals stay visible; sequence is priority (`You're in Fix`), not hiding. Step 3 leads with a **Your plan** timeline (when Fix finishes, when the emergency fund starts/completes, when each goal lands, when Invest saving can start, and how much sooner the goal lands vs cash-only if you invest this way) plus the vertical stage stack (Fix → Stabilize → Plan → Invest) with Living↔Net dials, hold status, sticky **You're in …**, and a % ring on each goal row. Living goals have **Sooner / Later** plus a vertical handle to drag the target month; calendar dates update on the timeline. Moving a goal stays in its stage and can extend that stage’s end (`max(months)`). Stages may overlap (Stabilize can start near the end of Fix; same for Plan/Invest). Other-goal % is given the renegotiation path + current EF. Honest “does not hold — yet” still works.

A small footer link (“Other tools from Plan Your Life”) is optional. It is not a chooser.

This repo’s workflow publishes `npm run build` to the `gh-pages` branch on each push to `main`. Pull requests also publish a Fortune preview at `https://betomarine.github.io/debtrenegociation1/preview/pr-N/fortune/` when the preview workflow runs. Completing a pack still does not POST names, HKID, amounts, or files — hosting is JS/CSS only.
...
4. **Step 3 · Your new life** — Living % vs Security-net % **above** the board, a one-line “holds” / “does not hold — yet”, sticky **You're in {stage}**, and four vertical sections **Fix → Stabilize → Plan → Invest**. **All stages start open** (sequence is priority, not hiding — Plan/Invest stay visible while Fix / EF are in progress). Headers still collapse on tap. Each goal is a **row with a % ring**. Stage header % is the average for that step, not a single goal. Stage end is **max(months)** of rows in that stage; stages **may overlap**. Living rows have always-visible **Sooner / Later** (44px) and a vertical handle to scrub `months` — dates stay on the row. User does not invent Fix; RD/Sunday own renegotiation tenor. **Invest** stays on the board even when thin (placeholder: “Suggested mix (after the emergency fund) — not a product”; no brokerage / fund picker). Silent house-mix marks stay a floor-honest ~5% overlay (locked cash 25 / FI 40 / stocks 25 / REIT 10) until Stabilize; after the floor holds they follow the selected Adjust card (Firm 12% / Balanced 15% / Growth 20% / Frontier 35%). Mix disclosure UI is out of scope. No multi-pin path chart as the home board. Coach is one line + 1–3 chips. **Adjust** holds mixes, ~4.5% price-rise, shuffle, save-vs-borrow, PDF, export. Live Finnhub marks and mix disclosure are **not** in this cut.
...
**Absurd input:** 0 income, 0 savings, a HK$15M+ house must hard-fail (dials wrecked, no green “you're set”). Covered by `src/fortune/engine.test.js`. The board coach stays: honest “does not hold — yet” plus delay / cut / add-money chips so it is never a dead end.
...
**Fortune Teller:** https://betomarine.github.io/debtrenegociation1/fortune/ → Fortune Teller (PoC v0.9.12). No chooser. Case ids live in `fortune/test-cases/`. **Bob** runs `npm test` before **Maddy** Safari QA. Live LOOP / ENTRY still need Safari. **FT-DATE-01** is a readable plan timeline (calendar dates, not tenor strings). **FT-EF-02** / **FT-INVEST-01** cover EF start/end and Invest start + growth vs cash-only. **FT-DRAG-01** / **FT-OVERLAP-01** are automated in this build (Sooner/Later + soft phase overlap); Safari confirms the gesture.
...
4. **Must-fix loop:** Heavy → **Open Right Door**. The **header** (sticky, above the product title — not only a footer link) shows **Back to Fortune Teller** in English while you work or leave the pack. Right Door footer is **PoC v0.8.0**. Tap the header chip. Fortune lands on **Me today** with **Looks right — open my plan**. Fortune footer must still read **PoC v0.9.12** (not Right Door’s **PoC v0.8.0**) with no hard refresh (**FT-VER-01**). Open the board: **Fix** is labeled **Debt renegotiation · 3 months** or **Debt renegotiation · 6 months** (pack tenor, or pick once). **Stabilize** is emergency fund with the **current amount** (0 allowed) — the 3/6-month renegotiation term is not the EF label. Plan/Invest stay **visible**. Same header loop for Sunday Pack fire link. Direct RD/Sunday URLs have no Fortune bar.
5. **Step 3:** a single **Your plan** timeline answers at a glance: when Fix finishes, when the emergency fund starts and completes, when each goal lands, and when Invest saving can start plus how much sooner a goal lands vs cash-only. Stage stack (Fix / Stabilize / Plan / Invest) stays below — not overlapping path pins. Living↔Net dials above; one-line hold status; sticky **You're in …**. All stages visible. Calendar dates on the timeline and on every row (never `3–6 months` as the only when). Fix keeps **Debt renegotiation · 3 or 6 months** on the title. Rebuild includes **Invest** (First growth pot / placeholder mix — not a product). Open **First growth pot**, change the amount, **Pin this goal**: the emergency-fund **now HK$** must not change, and the board must not grow a second First growth pot. **Time (FT-DRAG-01):** on a Plan row tap **Later** — the date and success % move on the timeline, the goal stays in Plan, and the stage **to {month}** extends if it was the last goal. **Sooner** pulls it earlier. Drag the ⋮⋮ handle up/down to scrub months (phone: handle stays visible). Footer must read **PoC v0.9.8**. **Overlap (FT-OVERLAP-01):** pull a Plan goal Sooner into the Fix window (Fix still 3/6 from RD). Plan stays Plan; Stabilize can already sit at the end of Fix. You cannot drag a goal into another stage or invent a Fix row. Percentages wait on “…” / last value — never a fake 0% flash. Hard-fail coach is **one** next action (Fix or floor).
...
Other checks: Sooner/Later on living rows; handle drag updates the date, not stage. **Adjust** → Firm → Frontier (Growth/Frontier only after the floor holds; Frontier warns before apply). Save-vs-borrow. PDF footer (not advice / not HSBC). Right Door and Sunday Pack URLs unchanged. Locked `/pyl/` DRAFT shelf unchanged (two Live tools; Fortune Testing, no Open).
