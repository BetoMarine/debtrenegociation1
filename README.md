...
Other checks: Sooner/Later on living rows; handle drag updates the date, not stage. **Adjust** → Steady → Frontier. Save-vs-borrow. PDF footer (not advice / not HSBC). Right Door and Sunday Pack URLs unchanged. Locked `/pyl/` DRAFT shelf unchanged (two Live tools; Fortune Testing, no Open).
