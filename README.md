...
**Fortune Teller:** https://betomarine.github.io/debtrenegociation1/fortune/ → Fortune Teller (PoC v0.9.9). No chooser. Case ids live in `fortune/test-cases/`. **Bob** runs `npm test` before **Maddy** Safari QA. Live LOOP / ENTRY still need Safari. **FT-DATE-01** is a readable plan timeline (calendar dates, not tenor strings). **FT-EF-02** / **FT-INVEST-01** cover EF start/end and Invest start + growth. **FT-DRAG-01** / **FT-OVERLAP-01** are automated in this build (Sooner/Later + soft phase overlap); Safari confirms the gesture.
